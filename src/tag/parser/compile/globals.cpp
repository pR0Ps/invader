...
namespace Invader::Parser {
    ...
    void GlobalsMultiplayerInformation::post_compile(BuildWorkload &workload, std::size_t tag_index, std::size_t struct_index, std::size_t offset) {
        ...
        if(SOUND_COUNT > HEK::MultiplayerInformationSound::MULTIPLAYER_INFORMATION_SOUND_TING) {
            const float TING_VOLUME = workload.build_parameters.engine_target == HEK::CacheFileEngine::CACHE_FILE_CUSTOM_EDITION ? 1.0F : 0.2F;
        ...
        if(SOUND_COUNT < HEK::MultiplayerInformationSound::MULTIPLAYER_INFORMATION_SOUND_ENUM_COUNT && !workload.build_parameters.hide_pedantic_warnings) {
